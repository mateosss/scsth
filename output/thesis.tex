...
\tightlist
\item
  Linear transforms and operators (skew symetric, cross product)
\item
  Rotation representations: euler, axis/angle, quaternion, 3x3 intro
\item
  Groups
\item
...
\item
  Mention Lie group/algebra (dont dive)
\item
  Exponential Map / Logarithm (and mention of rodrigues formula)
\item
  Same for SE(3): twist, exp, log, hat, vee
\item
  Pro: unconstrained optimization!!!!!!!
\end{enumerate}

Usualmente necesitaremos referirnos a transformaciones tridimensionales
y ser capaces de manipularlas. El tipo de transformación en el que
estamos interesados son los \emph{movimientos de cuerpo rígido}. Estos
pueden describirse mediante \emph{traslaciones} y \emph{rotaciones}. A
continuación, desarrollaremos algunos conceptos que nos ayudaran a
describir estas ideas más formalmente.

\hypertarget{operadores-uxfatiles}{%
\subsection{Operadores útiles}\label{operadores-uxfatiles}}

Definición: Producto punto Observación: producto punto es x\^{}t x
(mover lo que está en least squares acá referenciarlo allá) Definición:
Producto cruz Definición: Hat operator
...
\hypertarget{transformaciones-lineales-y-operadores}{%
\subsection{Transformaciones lineales y
operadores}\label{transformaciones-lineales-y-operadores}}

\hypertarget{representraciones-de-rotaciones}{%
\subsection{Representraciones de
rotaciones}\label{representraciones-de-rotaciones}}
...
\hypertarget{coordenadas-exponenciales}{%
\subsection{Coordenadas exponenciales}\label{coordenadas-exponenciales}}
...
\tightlist
\item
  General Linear Group: GL(n) = \{invertible mats, @\} = \{A / det(A) !=
  0\}
\item
  Special Linear Group: SL(n) = \{A in GL(n) / det(A) = 1\}
\item
  Orthogonal Group: O(n) = R orthogonal in M(n) = \{ R in M(n)/ RtR=I\}
  (=\textgreater{} det(R) = +-1)
\item
  Special Orthogonal Group: SO(n) = \{R in O(n) / det(R) = +1\}
  (rotation matrices)
\item
  Affine Group: A(n) = L:Rn-\textgreater Rn / L(x) = Ax + b con A in
  GL(n), b in Rn
\item
  Euclidean Group: E(n) = L:Rn-\textgreater Rn / L(x) = Rx + T con R in
  O(n), T in Rn
\item
  Special Euclidean Group: SE(n) = \{L(x) = Rx + T in E(n) / R in
  SO(n)\}
\end{itemize}

GL(n) in M(n) SL(n) in GL(n) SO(n) in SL(n) SO(n) in O(n) A(n) in GL(n +
1) E(n) in A(n) SE(n) in E(n) with R(n) in SO(n)

\begin{itemize}
\tightlist
\item
  citar https://ethaneade.com/lie\_groups.pdf (o
...
en su definición en el \Cref{lst:slam-tracker-def}. Lo que haremos para
