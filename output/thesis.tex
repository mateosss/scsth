...
tridimensionales entre distintos sistemas de referencias. Estos sistemas
surgen de las entidades que forman parte de nuestro proceso de
optimización en SLAM. Algunos ejemplos de transformaciones en los que
podríamos estar interesados son las que describen que transformación es
necesaria realizar sobre la cámara izquierda para llegar a la cámara
derecha de nuestro dispositivo, o la transformación que le ocurrió al
agente localizado entre el instante anterior y el actual.
...
espacio de tal entidad. A continuación, desarrollaremos algunos
conceptos que nos ayudarán a describir la idea de transformación más
formalmente, mientras que consideraremos a una pose como una
transformación aplicada sobre un origen.
...
\begin{remark}[Matriz de una transformación]Si \(V\) tiene base canónica
\(e_1, ..., e_n\) con \(e_i \in \R^m\) tenemos que la matriz
\(A = [L(e_1), ..., L(e_n)] \in \R^{m \times n}\) cumple
\(Ax = L(x) \quad \forall x \in V\)\end{remark}

\begin{remark}[Matrices cuadradas]El conjunto de matrices en
...
sobre el cuerpo de los reales. En particular, es un conjunto cerrado
bajo estas operaciones.\end{remark}

\begin{remark}\label{rmk:sqnormofsum}Sean \(x, y \in \R^n\), tenemos que
\(\| x + y \| ^ 2 = \|x\|^2 + \|y\|^2 + 2 x^T y\).\end{remark}
...
cuando solo se necesita describir una rotación, pero se vuelve dificil
en otros contextos.
...
en ese punto. Más problemas de los ángulos Euler se mencionan en
\textcite{shoemakeAnimatingRotationQuaternion1985}, secc 2.4.
...
necesaria ningún tipo de transformación. Además, veremos más abajo que
esta forma de describir rotaciones surge naturalmente en el estudio de
...
\hypertarget{matrices-de-rotaciuxf3n}{%
\subsubsection{Matrices de rotación}\label{matrices-de-rotaciuxf3n}}
...
ortonormales si \(\norm{v} = \norm{w} = 1\) y sus columnas son
ortogonales, es decir \(\dotprod{v}{w} = 0\)\end{definition}
...
de vectores en $\R^3$.
}. El conjunto estas matrices se denomina \(SO(3)\) (veremos en la
sección siguiente el por qué). Estas matrices presentan propiedades
agradables para ser manipuladas como rotaciones en el álgebra matricial
usual.
...
Más aún, el avance tecnológico en las unidades vectoriales, tanto en CPU
como GPU, hace que la manipulación de matrices sea usualmente igual o
más rápida que la de cuaterniones.
